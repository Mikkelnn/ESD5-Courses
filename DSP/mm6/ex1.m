...
filterOrder = [10 50 100]; % Filter order
...
function kaiserWindowPlot(Delta_f, Delta_p, M, N_bessel)
    ...
    n = 0:M(i); % Time samples
    
    % Compute the Kaiser window with Bessel approximation
    w = zeros(1, length(n)); % Initialize Kaiser window
    for p = 1:length(n)
        t = 2*(n(p) / M(i)) - 1; % Normalized time index
        w(p) = besselI0(beta * sqrt(1 - t^2), N_bessel) / besselI0(beta, N_bessel);
    end
    
    % Step 3: Plot the time domain representation
    stem(n, w, 'filled');
    title(['Kaiser Window (, Beta = ' num2str(beta) ')']);
    xlabel('n');
    ylabel('w[n]');
    grid on;
